...
def _save_dataset_train():
    # https://github.com/lorenzkuhn/semantic_uncertainty/blob/main/code/parse_coqa.py
    save_path = f'{_settings.DATA_FOLDER}/coqa_dataset_full'
    if not os.path.exists(save_path):
        # https://downloads.cs.stanford.edu/nlp/data/coqa/coqa-dev-v1.0.json
        with open(f'{_settings.DATA_FOLDER}/coqa-dev-v1.0.json', 'r') as infile:
            data = json.load(infile)['data']
...
def get_dataset_train(tokenizer, split='validation'):
    # from https://github.com/lorenzkuhn/semantic_uncertainty/blob/main/code/parse_coqa.py
    dataset = datasets.load_from_disk(_save_dataset())
    id_to_question_mapping = dict(zip(dataset['id'], dataset['question']))

    def encode_coqa_p(example):
        example['answer'] = answer = example['answer']['text']
        example['prompt'] = prompt = example['story'] + ' Q: ' + example['question'] + ' A:'
        prompt_encodings = tokenizer(prompt, truncation=True, padding='max_length', max_length=512)
        answer_encodings = tokenizer(answer, truncation=True, padding='max_length', max_length=512)
        return {"prompt": prompt_encodings, "answer": answer_encodings}

    dataset = dataset.map(encode_coqa_p, batched=False, load_from_cache_file=False)
    dataset.set_format(type='torch', columns=['prompt', 'answer'], output_all_columns=True)
