...
def load_model_and_tokenizer_rank(model_name='opt-13b', device='cuda:0', **kwargs):
    if model_name in {'gpt-3.5-turbo'}:
        return None, None
    if model_name in {"opt-2.7b", "opt-1.3b", "opt-6.7b", 'opt-13b'}:
        return load_model_and_tokenizer(f"facebook/{model_name}", device, **kwargs)
    if model_name.startswith('facebook/opt-'):
        return _load_pretrained_model(model_name, device, **kwargs), _load_pretrained_tokenizer(model_name)
    return _load_pretrained_model_rank(model_name, device, **kwargs), _load_pretrained_tokenizer(model_name)



def load_model_and_tokenizer_ats(model_name='opt-13b', device='cuda:0', **kwargs):
    if model_name in {'gpt-3.5-turbo'}:
        return None, None
    if model_name in {"opt-2.7b", "opt-1.3b", "opt-6.7b", 'opt-13b'}:
        return load_model_and_tokenizer(f"facebook/{model_name}", device, **kwargs)
    if model_name.startswith('facebook/opt-'):
        return _load_pretrained_model(model_name, device, **kwargs), _load_pretrained_tokenizer(model_name)
    return _load_pretrained_model_ats(model_name, device, **kwargs), _load_pretrained_tokenizer(model_name)
