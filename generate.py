...
import dataeval.hotpotqa as hotpotqa
import dataeval.cqa as cqa
import models
import utils
...
def get_dataset_fn(data_name):
    ...
    if data_name == "cqa":
        return cqa.get_dataset
    if data_name == "hotpotqa":
        return hotpotqa.get_dataset
...
def get_generation_config(input_ids, tokenizer, data_name):
    ...
    if data_name == 'cqa':
        generation_config = cqa._generate_config(tokenizer)
    if data_name == "hotpotqa":
        generation_config = hotpotqa._generate_config(tokenizer)
...
class ConstrainedLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids, scores):
        # Set logits of all tokens not in allowed_token_ids to a large negative value
...
def get_generations(model_name:str, args, seed=10, old_sequences=None, max_num_gen_once=4):
    ...
    sequences = []
    # allowed_tokens = tokenizer(['A', 'B', 'C', 'D', 'E'], add_special_tokens=False)['input_ids']
    # allowed_token_ids = [token[0] for token in allowed_tokens]
    # constrained_processor = ConstrainedLogitsProcessor(allowed_token_ids)
